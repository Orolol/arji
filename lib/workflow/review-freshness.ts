 ...
 * never parked.
 *
 * A NULL verdict on a provider Arij never wired a channel for still reads as
 * clean: its only verdict signal is the prose scan this gate never read,
 * which is why the rule turns on the CHANNEL and not on the empty verdict.
 */
function isCleanReviewSql(database: ArijDatabase): SQL {
  return sql`${agentSessions.status} = 'completed'
     AND ${agentSessions.userStoryId} IS NULL
     AND ${agentSessions.agentType} IN (${sql.raw(REVIEW_AGENT_TYPES_SQL)})
     AND ${agentSessions.outcome} = 'answered'
  ...
  return database
    .select({
      ...
      cleanReviewAt: sql<string | null>`CASE
        WHEN ${isCleanReviewSql(database)} THEN ${sessionAtSql()} END`.as(
        "clean_review_at"
      ),
