...
export function buildRetryDispatch(
  ...
  const agent = resolveRetryAgent(failed, selectedNamedAgentId);

  const body: RetryDispatch["body"] = {};
  if (agent.namedAgentId) body.namedAgentId = agent.namedAgentId;

  const resumeSessionId = resolveResumeTarget(failed, agent);
  if (resumeSessionId) body.resumeSessionId = resumeSessionId;

  return { url, body };
}

interface RetryAgentChoice {
  namedAgentId: string | null;
  /**
   * True only when namedAgentId was carried over from the failed session
   * itself. A toolbar selection that happens to name the same agent is NOT a
   * continuation, so this cannot be re-derived by comparing ids — that
   * inference is what let a build retry resume a review conversation.
   */
  reusedFromFailure: boolean;
}
 ...
 * conversation is story-shaped — see resolveResumeTarget.
 */
function resolveRetryAgent(
  failed: FailedSessionInfo | undefined,
  selectedNamedAgentId: string | null,
): RetryAgentChoice {
  if (failed?.namedAgentId && isCodeProducingAgentType(failed.agentType)) {
    return { namedAgentId: failed.namedAgentId, reusedFromFailure: true };
  }
  return {
    namedAgentId: selectedNamedAgentId || null,
    reusedFromFailure: false,
  };
}

/**
 * The session id to continue, or null to start cold.
 *
 * Returning the id rather than a boolean lets the caller narrow it without a
 * non-null assertion, so the compiler enforces what this gate decided.
 *
 * The server re-checks part of this (validateResumeSession) and silently
 * starts fresh when it disagrees; deciding it here too keeps the request
 * honest about its intent — and covers the cases the server cannot see from
 * the epic route, which compares provider, epic scope and cliSessionId but
 * never the agent type or the story.
 */
function resolveResumeTarget(
  failed: FailedSessionInfo | undefined,
  agent: RetryAgentChoice,
): string | null {
  if (!failed?.sessionId) return null;

  // The retry must actually BE a continuation of the failed session — same
  // agent, carried over from that session. Anything else (the toolbar's pick,
  // the server's default chain) is a different process that has no claim on
  // the stored CLI session id. This is also what keeps a review or grading
  // thread out of a build retry: resolveRetryAgent refuses to carry those
  // over, so reusedFromFailure is false for them however the toolbar is set.
  if (!agent.reusedFromFailure) return null;
  ...
  if (!failed.provider || !isResumableProvider(failed.provider)) return null;
  ...
  if (failed.userStoryId) return null;
  ...
  return failed.producedOutput ? failed.sessionId : null;
}
