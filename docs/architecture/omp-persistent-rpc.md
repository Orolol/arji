...
The process is launched with `--mode rpc` and the existing read-only OMP tool allowlist. It passes **no** `--config` overlay: the `tools.xdev: false` overlay it used to carry is a measured no-op on omp 18.0.6, and `--config` is a lever over the user's whole `~/.omp/agent/config.yml` — see `docs/architecture/mcp-provider-matrix.md`. Because the allowlist is then the whole isolation mechanism, the adapter's `preflight` refuses the turn outright on an omp below `OMP_MIN_ALLOWLIST_VERSION` (18.0.6) — before the child spawns and before the MCP token is minted. Its per-conversation MCP token uses OMP's environment expansion and is revoked only when the warm process exits.
